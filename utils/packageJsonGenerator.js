...
module.exports = function packageJsonGenerator(existingConfig, options = {}) {
    ...
    if (targetConfig && typeof targetConfig === 'object') {
        ...
        targetConfig.description = or(targetConfig.description, options.description);
        // custom
        ...
        targetConfig.module = or(targetConfig.module, `dist/mjs/${ext(options.module)}`);
        ...
        if (hasOwn(existingScripts, 'start')) {
            console.log(colors.yellow(`[INFO]: Found an existing ${colors.bold('"start"')} script. Renamed to ==> ${colors.bold('"rn:start"')}`));
            existingScripts['rn:start'] = existingScripts.start;
        }
        if (hasOwn(existingScripts, 'build')) {
            console.log(colors.yellow(`[INFO]: Found an existing ${colors.bold('"build"')} script. Renamed to ==> ${colors.bold('"rn:build"')}`));
            existingScripts['rn:build'] = existingScripts.build;
        }
        if (hasOwn(existingScripts, 'test')) {
            console.log(colors.yellow(`[INFO]: Found an existing ${colors.bold('"test"')} script. Renamed to ==> ${colors.bold('"rn:test"')}`));
        ...
        if (missingFields.length) {
            console.log(
                colors.yellow(`[Warning]: Field(s) "${colors.bold(missingFields.join(', '))}" in "package.json" are missing. \nRun ${colors.bold('"npm init"')} to add missing field(s).`)
            );
        }
        return targetConfig;
    }
